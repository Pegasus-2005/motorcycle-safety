...
def load_data(path):
    ...
    df['sensorLeft'] = np.logical_or(df.SwitchSidesensor_left_road, df.SwitchSidesensor_left_car)
    df['sensorRight'] = np.logical_or(df.SwitchSidesensor_right_road, df.SwitchSidesensor_right_car)
    df['FW_cnt_Force'] = df.FW_Car_cnt_force + df.FW_Road_cnt_force
    df['RW_cnt_Force'] = df.RW_Car_cnt_force + df.RW_Road_cnt_force

    for col in ['sensorLeft', 'sensorRight']:
...
def evaluate_control_scenarios(model):
    ...
    for fname in all_files:
        ...
        metrics["is_crash_scenario"] = int("iso" in lower_name)

        rows.append(metrics)

    out_df = pd.DataFrame(rows)
    out_df = out_df[
        ["scenario", "is_crash_scenario", "tn", "fp", "fn", "tp", "accuracy", "precision", "recall", "f1", "roc_auc", "false_positive_rate"]
    ]
    ...
    for fname in test_paths:
        ...
        metrics = metrics_dict(y, pred, prob)
        metrics["scenario"] = fname
        metrics["is_crash_scenario"] = int("ISO" in fname or "Iso" in fname)
        rows.append(metrics)
    ...
    print("\nCONTROL SCENARIO RESULTS")
    print(out_df[["scenario", "is_crash_scenario", "fp", "tp", "accuracy", "precision", "recall", "f1"]])
    print(f"\nSaved: {out_path}")
